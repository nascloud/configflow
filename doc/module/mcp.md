...
| 已在「系统设置 → 配置令牌」设置了令牌 | 该配置令牌 |
| 已启用账号密码登录 | 登录后获得的 JWT，或配置令牌 |
...
> 对外网可访问的部署，建议先在「系统设置 → 配置令牌」生成一个令牌再接入。
...
| `generate_config` | 生成配置并保存，订阅链接随即生效 |
...
- **改完配置记得生成**：修改订阅、规则或策略组后，需要调用 `generate_config` 才会写入订阅链接对应的配置文件。
...
| 现象 | 原因与处理 |
|------|-----------|
| 客户端提示 401 | 未带凭证或凭证不对，检查配置令牌是否与「系统设置」中一致 |
