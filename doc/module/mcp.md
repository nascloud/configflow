...
| 未启用账号密码登录，但设置了配置令牌 | 该配置令牌 |
| 已启用账号密码登录 | **只接受**登录后获得的 JWT |
...
> ⚠️ **启用了账号密码登录时，配置令牌不能用于 MCP。**
> 配置令牌是随订阅链接分发到各个代理客户端设备上的只读凭证，而 MCP 工具可以
> 导出整份配置、重置系统、卸载 Agent。若允许它调用 MCP，等同于把管理员权限
> 发给了每一台装了订阅链接的设备，因此这条路径被明确禁止。
>
> 相应地，此时 MCP 使用的 JWT 有效期为 24 小时，过期后需要重新登录获取。
> 如果需要长期稳定的接入凭证，建议后续增加专用的 MCP 令牌。
...
| 现象 | 原因与处理 |
|------|-----------|
| 客户端提示 401 | 未带凭证或凭证不对；若已启用账号密码登录，请确认用的是登录 JWT 而非配置令牌 |
| 接入一段时间后开始 401 | 启用账号密码登录时 MCP 用的是 JWT，有效期 24 小时，需重新登录获取 |
